...
void opcion_8(){
    ...
    // Usamos stringstream para construir el string
    std::ostringstream ss;
    ss << "Temps: ";

    
    // 'g_temp_index' apunta al slot donde se escribirá el *próximo* dato,
    // que es también la ubicación del dato *MÁS ANTIGUO*.
    // Empezamos desde el más antiguo y damos 8 vueltas.
    
    for (int i = 0; i < 8; i++) {
        // Calculamos el índice a imprimir, rotando alrededor del array
        int print_index = (g_temp_index + i) % 8;
        
        ss << g_temperaturas[print_index] << "C ";
    }
